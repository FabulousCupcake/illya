...
const setGameAccountId = async (discordId, gameAccountId) => {
  const key = `gameAccountId-${discordId}`;
  await redisClient.async.set(key, gameAccountId);
}

const getGameAccountId = async (discordId) => {
  const key = `gameAccountId-${discordId}`;
  return await redisClient.async.get(key);
}
...
const setStickyMessageId = async (messageId) => {
  const key = `stickyMessageId`;
  await redisClient.async.set(key, messageId);
}

const getStickyMessageId = async () => {
  const key = `stickyMessageId`;
  return await redisClient.async.get(key);
}
...
const isPilot = async (discordId) => {
  const key = `pilot-${discordId}`;
  const val = await redisClient.async.get(key);
  return !!val;
}
